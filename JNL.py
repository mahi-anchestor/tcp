...
chat_ip = "202.81.108.103"
chat_port = 39699
key2 = "JENILxAPI"
#Don ' t Change Api Key 🔐 
...
def newinfo(uid):
    try:
        response = requests.get(f"https://zoroinfomain.vercel.app/player-info?uid={uid}&region=ind")
...
def send_likes(uid):
    try:
        likes_api_response = requests.get(
            f"[Your Like API With Endpoit]uid={uid}&server_name=ind"
        )
...
[FFFFFF]Number of Likes Added : [00FF00]{likes_added}  
[FFFFFF]Likes Before: [00FF00]{likes_before}  
[FFFFFF]Likes After: [00FF00]{likes_after}  
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Code By:  [00FF00]JNL CODEX!!
  
                """
            else:
                message = (
                    f"[C][B][FF0000]________________________\n"
                    f"You have reached your daily limit of likes. Try again in 24 hours.\n"
...
def get_jwt_token():
    global jwt_token
    url = "https://jnl-gen-jwt.vercel.app/token?uid=3985009787&password=1589513234B9C853E656A0BDEE60308C863885935981BD7CC0C96AB5411BFAFF"
    try:
...
[FFFFFF]How To Create Your Own Bot?
[00FFFF]Video On YouTube: [FF1493]JNL CODEX 
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Code By: JNL CODEX!!
        """
        return time
...
def spam_requests(player_id):
        url = f"https://jnl-spam-ind.vercel.app/spam-jenil?uid={player_id}&key=JENILxAPI"
        res = requests.get(url)
...
[FFFFFF]Friend requests sent: [00FF00]{succspam}
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Code By:  [00FF00]JNL CODEX!!
"""
            return msg_spam
        else:
            return "Failed to spam friend requests"


def Increase_visits(player_id):
    url = f"https://danger-visit.vercel.app/ind/{player_id}key=DANGERxVISIT"
    res = requests.get(url)
    if res.status_code == 200:      
        data = res.json()
        uid = data["UID"]
    ...
    """
        ...
        return "Failure to increase player visits"
...
[FFFFFF]Account area: {region}
‎━━━━━━━━━━━━
[808080][B][C]Code By: JNL CODEX!!
    """
        return msg_visit
    else:
            ...
            if senthi == True:
                ...
                clients.send(
                        self.GenResponsMsg(
                            f"""
[C][B][00FFFF]━━━━━━━━━━━━━━━
[FFFFFF]Hello, how are you? Thank you for accepting my friend request! Send me an emoji to find out what you need to do.
[C][B][00FFFF]━━━━━━━━━━━━━━━
""", idinv
                        )
                )
                    ...
                    if uexmojiii == "DefaultMessageWithKey":
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                f"""
[C][B][00FFFF]━━━━━━━━━━━━━━━
[FFFFFF]Do you want to make a private server in your name?

[FFFFFF]To support your project and get a strong and stable server, See Yt Video Tutorial JNL Ab directly.

[00FF00] Youtube: @JNL_CODEX07


[FFFFFF]Did you see the orders? Send:
[00FF00]/🗿help

[C][B][00FFFF]━━━━━━━━━━━━━━━
[C][B][808080]Code By: JNL CODEX!!
""",uid
                            )
                        )
                else:
...
[11EAFD][B][C]
‎━━━━━━━━━━━━
[FFFFFF]Current status:spam requests send to the player.
...
[808080][B][C]Code By: JNL CODEX!!
            ...
            if "1200" in data.hex()[0:4] and b"/ai" in data:
                try:
                    ...
                    parsed_data = json.loads(json_result)
                    uid = parsed_data["5"]["data"]["1"]["data"]                    
                    clients.send(self.GenResponsMsg("We are connecting with the server, please wait...", uid))
                    try:
...
[FFFFFF][B]Create Your Own Bot
[1E90FF]YT : @JNL_CODEX07

[FFFFFF][B]Fetch player information:
[00CED1]/🗿info104[c]145[c]933[c]49

[FFFFFF][B]Know the player's status:
[DC143C]/🗿status/104[c]145[c]933[c]49

[FFFFFF][B]Find out if a player is blocked:
...
[FFFFFF][B]Increase the number of account visitors:
...
[FFFFFF][B]Knowing the player's server:
[1E90FF]/🗿region/104[c]145[c]933[c]49

[C][B][FFB300]Create Your Own Bot See Tutorial On YouTube:
[00FF00]JNL CODEX
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="3985009787", password="1589513234B9C853E656A0BDEE60308C863885935981BD7CC0C96AB5411BFAFF")
        client_thread.start()
    except Exception as e:
